...
int main(void) {
  ...
  unsigned int dphase =
      (unsigned int)(10.0 / FREQ * PWM_LEN * (1 << 16) * (1 << 16));  // 90.0
  double dphase_inc;
  ...
  for (i = 0; i < SIM_TIME * FREQ; i++) {
    ...
    sineU_mod = floor((cos(2 * 3.14159 * sineU_ix / PWM_LEN) + 1) / 2 * AMP *
                      PWM_LEN * (i / (SIM_TIME * FREQ) + AMP_OFFSET));
    sineV_mod = floor((cos(2 * 3.14159 * sineV_ix / PWM_LEN) + 1) / 2 * AMP *
                      PWM_LEN * (i / (SIM_TIME * FREQ) + AMP_OFFSET));
    sineW_mod = floor((cos(2 * 3.14159 * sineW_ix / PWM_LEN) + 1) / 2 * AMP *
                      PWM_LEN * (i / (SIM_TIME * FREQ) + AMP_OFFSET));
    ...
    // Rotor
    rotorAlpha = sin(U_VECT - rotorAngle) * iu * KE / INERTIA * POLES * POLES;
    rotorAlpha += sin(V_VECT - rotorAngle) * iv * KE / INERTIA * POLES * POLES;
    rotorAlpha += sin(W_VECT - rotorAngle) * iw * KE / INERTIA * POLES * POLES;

    rotorSpeed += (rotorAlpha / FREQ);
    rotorAngle += (rotorSpeed / FREQ);

    if ((i % PWM_LEN) == 0) {
      dphase_inc = ((float)dphase * i / (SIM_TIME * FREQ));
      if (0xFFFFFFFF - phase < dphase_inc) phase_revs++;
      phase += dphase_inc;
    }

    if ((i % 400) == 0) {
      deltaAngle_rad =
          2 * PI * (phase_revs + phase / (1 << 16) / (1 << 16)) - rotorAngle;
